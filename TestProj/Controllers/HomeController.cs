...
{
    // Remove collections from ModelState so EF Core doesn't complain
    ModelState.Remove(nameof(MuseumModel.TicketTypes));
    ModelState.Remove(nameof(MuseumModel.Orders));
    ModelState.Remove(nameof(MuseumModel.Employees));
    ModelState.Remove(nameof(MuseumModel.Images));

    // Handle OpeningTime/ClosingTime parsing from form
    if (Request.Form.TryGetValue("OpeningTime", out var openingVal))
    {
        if (TimeSpan.TryParse(openingVal, out var ts))
            museum.OpeningTime = ts;
        else
            museum.OpeningTime = TimeSpan.Zero;
    }
    ...
    if (Request.Form.TryGetValue("ClosingTime", out var closingVal))
    {
        if (TimeSpan.TryParse(closingVal, out var ts))
            museum.ClosingTime = ts;
        else
            museum.ClosingTime = TimeSpan.Zero;
    }
    ...
    // Handle TicketTypes: ensure the Museum reference is set for EF Core
    if (museum.TicketTypes != null)
    {
        foreach (var tt in museum.TicketTypes)
        {
            tt.MuseumId = museum.MuseumId;
        }
    }

    // Handle Images: ensure at least one image exists
    if (museum.Images == null || !museum.Images.Any())
    {
        museum.Images = new List<MuseumImageModel>
        {
            new MuseumImageModel { ImageUrl = "/images/imagenotfound.png" }
        };
    }
